...
class HomePageInfoSchema(BaseModel):
    """Schema for complete homepage info"""
    stats: List[StatItemSchema] = Field(default_factory=list, description="Stats array")
    testimonials: List[TestimonialItemSchema] = Field(default_factory=list, description="Testimonials array")
    contact: ContactInfoSchema = Field(default_factory=dict, description="Contact information")


class StatsUpdateSchema(BaseModel):
    """Schema for updating stats"""
    stats: List[StatItemSchema] = Field(..., description="Stats array")


class TestimonialsUpdateSchema(BaseModel):
    """Schema for updating testimonials"""
    testimonials: List[TestimonialItemSchema] = Field(..., description="Testimonials array")
