"""
HomePageInfoRepository — dino-system.

Manages the singleton homepage_info table.
Uses a get-or-create pattern: SELECT first, INSERT only when the row is absent.
"""
...
class HomePageInfoRepository(BaseRepository):
    """Repository for the singleton homepage_info row."""
    ...
    async def _fetch_row(self) -> Optional[HomePageInfo]:
        """Return the first (and only) homepage_info ORM instance, or None."""
        stmt = select(HomePageInfo).limit(1)
    ...
    async def get_or_create_homepage_info(self) -> dict:
        """Return the singleton row as a dict, creating it with defaults if absent.

        Uses INSERT ... ON CONFLICT DO NOTHING so concurrent requests never
        race to insert a duplicate row.  The row is always fetched after the
        upsert to return the authoritative persisted state.
        """
        stmt = (
