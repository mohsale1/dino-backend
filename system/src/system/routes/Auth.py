...
async def login(request: LoginRequest):
    auth_service = SystemAuthService()
...
async def get_current_user(user: Dict[str, Any] = Depends(get_current_system_user)):
    return {'success': True, 'message': 'User retrieved successfully', 'data': user}

@router.post('/change-password', response_model=BaseResponse)
async def change_password(request: ChangePasswordRequest, user: Dict[str, Any] = Depends(get_current_system_user)):
    auth_service = SystemAuthService()
    try:
