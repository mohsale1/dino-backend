...
from app.models.entities import OrderStatus, PaymentStatus, OrderType
...
class PublicOrderingService:
    ...
    async def create_public_order(self, order_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            order_id = generate_document_id()
            order_number = self._generate_order_number()
            
            # Create or update customer record
            customer_info = order_data.get('customer', {})
            customer_id = await self._handle_customer_record(customer_info, order_data['venue_id'])
            ...
            # Update customer statistics after successful order creation
            await self._update_customer_stats(customer_id, validation['total_amount'])
    ...
    async def _handle_customer_record(self, customer_info: Dict[str, Any], venue_id: str = None) -> str:
        """
        Create or update customer record and track venue IDs
        """
        try:
            ...
            if customer:
                ...
                # Add venue_id to customer's venue_ids list if not already present
                if venue_id:
                    venue_ids = customer.get('venue_ids', [])
                    if venue_id not in venue_ids:
                        venue_ids.append(venue_id)
                        update_data['venue_ids'] = venue_ids
                        logger.info(f"Added venue {venue_id} to customer {customer['id']} venue list")
                ...
                logger.info(f"Created new customer {customer_id} with venue {venue_id}")
                return customer_id
    ...
    async def _update_customer_stats(self, customer_id: str, order_amount: float):
        ...
        try:
            if customer_id == "guest-customer":
                # Skip stats update for guest customers
                return
