...
from app.models.entities import Table, TableStatus
...
# =============================================================================
# TABLE AREA MANAGEMENT ENDPOINTS
# =============================================================================
...
async def get_venue_areas(
    ...
    try:
        from app.database.repository_manager import get_venue_repo, get_table_area_repo
        venue_repo = get_venue_repo()
...
async def create_area(
    ...
    try:
        ...
        # Validate venue exists and user has access
        venue_id = area_data.get('venue_id')
        venue = await venue_repo.get_by_id(venue_id)
        if not venue:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
        ...
        existing_area = await area_repo.get_by_name(venue_id, area_name)
        if existing_area:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        area_dict = {
            ...
            'is_active': area_data.get('active', True)
        }
        
        # Create area in database
        created_area = await area_repo.create(area_dict)
        
        logger.info(f"Area created: {created_area['id']} for venue {venue_id}")
        return ApiResponseDTO(
            success=True,
...
async def get_area_by_id(
    ...
    try:
        from app.database.repository_manager import get_table_area_repo
        area_repo = get_table_area_repo()
...
async def update_area(
    ...
    try:
        ...
        # Validate venue exists and user has access
        venue_id = existing_area['venue_id']
        await tables_endpoint._validate_venue_access(venue_id, current_user)
        ...
        if 'name' in area_data:
            ...
            if area_data['name'] != existing_area.get('name'):
                existing_name_area = await area_repo.get_by_name(venue_id, area_data['name'])
        ...
        if 'active' in area_data:
            update_dict['is_active'] = area_data['active']
...
async def delete_area(
    ...
    try:
        ...
        repo = get_table_repo()
        tables_in_area = await repo.query([("area_id", "==", area_id)])
        if tables_in_area:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
async def get_area_tables(
    ...
    try:
        ...
        repo = get_table_repo()
        tables = await repo.query([("area_id", "==", area_id)])
