...
async def register_workspace(registration_data: WorkspaceRegistrationDTO):
    ...
    try:
        from app.database.repository_manager import (
            get_workspace_repo, get_venue_repo, get_user_repo, get_role_repo
        )
        ...
        try:
            ...
            logger.info(f"Password hashed for email: {registration_data.owner_email}")
        except ValueError as e:
            logger.warning(f"Password validation error for {registration_data.owner_email}: {e}")
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=str(e)
            )
        ...
        try:
            ...
            return ApiResponseDTO(
                ...
                data={
                    "workspace": {"id": actual_workspace_id, "name": workspace_name},
                    "venue": {"id": actual_venue_id, "name": registration_data.venue_name},
                    "owner": {
                        "id": actual_user_id,
                        "first_name": registration_data.owner_first_name,
                        "last_name": registration_data.owner_last_name,
            ...
            logger.error(f"Registration failed during creation: {creation_error}", exc_info=True)
            # Attempt cleanup
            try:
...
async def login_user(login_data: UserLoginDTO):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Login failed due to an unexpected error. Please try again."
        )
...
async def change_password(
    ...
    try:
        ...
        if not verify_password(request_data.current_password, stored_hash):
